...
public class Arrow {
    ...
    public Arrow(float x, float y, boolean facingRight, AnimationManager arrowAnimManager) {
        ...
        this.animationManager.setState("arrow", true); // key cho animation
        this.hitbox = new Rectangle(x, y, 32, 16); // size tuỳ chỉnh
    }

    public void update(float deltaTime, List<Enemy> enemyList, int arrowDamage) {
        if (isDestroyed) return; // Không xử lý nếu đã bị hủy
    ...
    public void render(SpriteBatch batch) {
        ...
        if (frame != null) {
            ...
            batch.draw(frame, drawX, position.y, width * scaleX, height);
        }
    }
